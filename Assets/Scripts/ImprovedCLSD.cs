...
public class ImprovedCLSD : MonoBehaviour
{
    [System.Serializable]
    public class ImprovedLyricLine {
    ...
    private List<ImprovedLyricLine> lyrics;

    void ParseLyrics() {
        lyrics = new List<ImprovedLyricLine>();
        ...
        foreach (string line in lines) {
            ...
            if (float.TryParse(parts[0], out float duration)) {
                lyrics.Add(new ImprovedLyricLine { duration = duration, text = parts[2].Trim(), rightSide = (parts[1] == "r") });
            }
        }
    }
