...
public class MassEliminationLevel3 : MonoBehaviour
{
    ...
    public IEnumerator MoveCamera() {
        ...
        while (elapsed < duration) {
            camera1.transform.position += new Vector3(0, -1.39f * Time.deltaTime, 99 * Time.deltaTime);
            camera1.transform.localRotation = Quaternion.Slerp(rotationStart, targetRotation, elapsed / duration);
            playersRemaining.text = $"Players Remaining:\n{(int) Mathf.Lerp(163, 32, elapsed / duration)}";
            if (audioBlend) {
        ...
        for (int x = 0; x < 3; x++)
        {
            for (int z = 0; z < 44; z++)
            {
                if (x != 1 || z != 0) {
                    ...
                    ep.eliminate = false;
                    ep.drop = true;
                    ep.delay = 2f + (Mathf.Abs(x - 1) + z) / 6f;
                }
            }
        }
    }
